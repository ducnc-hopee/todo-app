export function ViewTaskModal({ onClose }: { onClose: () => void }): void {
  const viewTaskModalContainer = document.getElementById('viewTaskModalContainer') as HTMLDivElement;

  if (!viewTaskModalContainer) {
    console.error('Không tìm thấy phần tử với ID "viewTaskModalContainer"');
    return;
  }
